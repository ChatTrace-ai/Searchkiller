  ...
  return (
    <main className="mx-auto max-w-[1500px] px-5 py-7 lg:px-8">
      <nav className="mb-5 text-sm text-slate-500">
        ...
        <span className="text-slate-800">{prediction.question}</span>
      </nav>
        ...
        <section className="min-w-0 rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
            <div className="flex min-w-0 items-center gap-4">
              <span className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-[radial-gradient(circle_at_35%_30%,#60a5fa,#1d4ed8)] text-white shadow-inner">
                <Gauge className="h-10 w-10" />
              </span>
              <div>
                <p className="text-sm font-semibold uppercase text-blue-600">{prediction.category}</p>
            ...
            <div className="w-full rounded-lg border border-slate-200 p-4 lg:w-64">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <span className="text-2xl font-bold text-slate-900">{prediction.confidence.score}</span>
              </div>
              <p className="mt-3 text-sm leading-5 text-slate-500">{prediction.confidence.explanation}</p>
            </div>
          </div>
                  ...
                  >
                    <td className="px-2 py-4">
                      <span className={`grid h-8 w-8 place-items-center rounded-md font-semibold ${
                        outcome.rank === 1 ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'
                      }`}>
                        {outcome.rank}
                      </span>
                    </td>
                    ...
                    <td className="px-2 py-4">
                      <span className="flex items-center gap-4">
                        <strong className="w-16 text-lg text-blue-600">{outcome.probability}%</strong>
          ...
          <div className="mt-7 space-y-3 md:hidden">
            {visibleOutcomes.map((outcome) => {
              const positive = outcome.change >= 0;
              return (
                <div key={outcome.id} className="rounded-md border border-slate-100 p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <strong className="text-lg text-blue-600">{outcome.probability}%</strong>
                  </div>
          ...
          {prediction.outcomes.length > 6 && (
            <button
              ...
              className="mx-auto mt-6 flex h-11 min-w-72 items-center justify-center gap-2 rounded-md border border-slate-200 font-semibold text-slate-800 hover:border-blue-300"
            >
              ...
              {prediction.sources.map((source) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-2 font-semibold text-slate-900 group-hover:text-blue-600">
                      {source.title}
                      <ExternalLink className="h-3.5 w-3.5" />
                    </span>
                    <span className="mt-1 block text-sm leading-5 text-slate-500">{source.description}</span>
                  </span>
