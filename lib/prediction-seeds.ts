import type { ConfidenceLevel, PredictionDetail } from './prediction-types';
...
export const RESULT_TTL_MS = 24 * 60 * 60 * 1_000;
...
export function buildDetail(seed: PredictionSeed, now = MOCK_UPDATED_AT): PredictionDetail {
  const listedTotal = seed.outcomes.reduce((sum, o) => sum + o.probability, 0);
  const completeOutcomes = [...seed.outcomes];

  if (listedTotal < 99.9) {
    completeOutcomes.push({ label: 'Other outcomes', probability: Number((100 - listedTotal).toFixed(1)), icon: '•' });
  }
  ...
  return {
    ...
    icon: seed.icon,
    status: 'completed',
    confidence: { level: 'high', score: 82, explanation: 'Stable signals across multiple independent information sources.' },
    outcomes,
...
export const seeds: PredictionSeed[] = [
