...
| 层级 | 选型 | 理由 |
|------|------|------|
| 框架 | Next.js 16 App Router | RSC + Turbopack，一键 Docker 部署 |
| 语言 | TypeScript + React 19 | 高频流式状态更新下的类型安全 |
...
│
├── lib/                    ← 纯共享工具库（零副作用）
│   ├── gemini.ts              Gemini Flash + Pro 模型实例（Vertex AI）
│   ├── exa.ts                 Exa.ai 语义搜索客户端（惰性初始化）
│   ├── elasticsearch.ts       ES 混合检索客户端（惰性初始化）
│   ├── schemas.ts             Zod 4 Schema + TypeScript 接口
│   └── context-cache.ts       研究会话内存缓存（API 路由间共享）
│
├── agents/                 ← 多智能体编排逻辑
